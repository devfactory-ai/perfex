/**
 * Healthcare Seed Data Generator for Staging V3
 * Uses existing company, organization, and user IDs from staging
...
// Use existing IDs from staging database
...
const contactIds = [];
for (let i = 0; i < 150; i++) {
  const id = `hc-contact-${String(i + 1).padStart(4, '0')}`;
  contactIds.push(id);
  const firstName = randomChoice(firstNames);
  const lastName = randomChoice(lastNames);

  sql.push(`INSERT OR IGNORE INTO contacts (id, organization_id, first_name, last_name, email, phone, status, created_by, created_at, updated_at) VALUES ('${id}', '${ORG_ID}', '${firstName}', '${lastName}', '${firstName.toLowerCase()}.${lastName.toLowerCase()}${i}@email.com', '+212${randomInt(600000000, 699999999)}', 'active', '${USER_ID}', ${now}, ${now});`);
}
...
for (let i = 0; i < 150; i++) {
  const id = `hc-patient-${String(i + 1).padStart(4, '0')}`;
...
for (let i = 0; i < 120; i++) {
  const id = `dial-patient-${String(i + 1).padStart(4, '0')}`;
...
for (let i = 0; i < 25; i++) {
  const id = `dial-machine-${String(i + 1).padStart(3, '0')}`;
  machineIds.push(id);
...
for (let i = 0; i < slots.length; i++) {
  const slot = slots[i];
  const id = `dial-slot-${String(i + 1).padStart(3, '0')}`;
  slotIds.push(id);
  sql.push(`INSERT OR IGNORE INTO dialyse_session_slots (id, organization_id, name, start_time, end_time, days_of_week, max_patients, active, created_at, updated_at) VALUES ('${id}', '${ORG_ID}', '${slot.name}', '${slot.start}', '${slot.end}', '${slot.days}', ${randomInt(8, 15)}, 1, ${now}, ${now});`);
}
...
for (let i = 0; i < dialysePatientIds.length; i++) {
  const id = `dial-rx-${String(i + 1).padStart(4, '0')}`;
...
const sessionIds = [];
for (let i = 0; i < 600; i++) {
  const id = `dial-sess-${String(i + 1).padStart(5, '0')}`;
  sessionIds.push(id);
...
for (let i = 0; i < dialysePatientIds.length; i++) {
  const id = `dial-access-${String(i + 1).padStart(4, '0')}`;
  const patientId = dialysePatientIds[i];
  const creationDate = randomTimestamp(new Date('2018-01-01'), new Date('2024-06-01'));

  sql.push(`INSERT OR IGNORE INTO dialyse_vascular_accesses (id, organization_id, patient_id, type, location, creation_date, surgeon, status, last_control_date, next_control_date, notes, created_by, created_at, updated_at) VALUES ('${id}', '${ORG_ID}', '${patientId}', '${randomChoice(accessTypes)}', '${randomChoice(locations)}', ${creationDate}, 'Dr. ${randomChoice(lastNames)}', '${randomChoice(['active', 'active', 'active', 'monitoring', 'failed'])}', ${randomTimestamp(new Date('2024-06-01'), new Date('2024-11-30'))}, ${randomTimestamp(new Date('2025-01-01'), new Date('2025-06-30'))}, '${escapeSQL(randomChoice(['Bon debit', 'Acces fonctionnel', 'Surveillance thrill', 'A surveiller']))}', '${USER_ID}', ${now}, ${now});`);
}

// DIALYSE LAB RESULTS
console.log('Generating lab results...');
for (let i = 0; i < 400; i++) {
  const id = `dial-lab-${String(i + 1).padStart(5, '0')}`;
  const patientId = randomChoice(dialysePatientIds);
  const labDate = randomTimestamp(new Date('2024-01-01'), new Date('2024-12-15'));

  sql.push(`INSERT OR IGNORE INTO dialyse_lab_results (id, organization_id, patient_id, lab_date, urea_pre, urea_post, creatinine, kt_v, hemoglobin, hematocrit, pth, calcium, phosphorus, potassium, sodium, albumin, ferritin, has_out_of_range_values, notes, created_by, created_at, updated_at) VALUES ('${id}', '${ORG_ID}', '${patientId}', ${labDate}, ${randomFloat(80, 180, 1)}, ${randomFloat(20, 60, 1)}, ${randomFloat(400, 1200, 0)}, ${randomFloat(1.0, 1.8, 2)}, ${randomFloat(8.0, 13.0, 1)}, ${randomFloat(25, 42, 1)}, ${randomFloat(50, 800, 0)}, ${randomFloat(2.0, 2.6, 2)}, ${randomFloat(3.0, 7.0, 1)}, ${randomFloat(3.5, 6.5, 1)}, ${randomFloat(135, 145, 0)}, ${randomFloat(30, 45, 1)}, ${randomFloat(100, 800, 0)}, ${randomChoice([0, 0, 0, 1])}, '${escapeSQL(randomChoice(['Valeurs dans les normes', 'A surveiller', 'Anemie legere', 'PTH elevee']))}', '${USER_ID}', ${now}, ${now});`);
}

// DIALYSE MACHINE MAINTENANCE
console.log('Generating machine maintenance...');
for (let i = 0; i < 80; i++) {
  const id = `dial-maint-${String(i + 1).padStart(4, '0')}`;
  const machineId = randomChoice(machineIds);
...
const pacemakerIds = [];
for (let i = 0; i < 40; i++) {
  const id = `cardio-pm-${String(i + 1).padStart(4, '0')}`;
  pacemakerIds.push(id);
  const patientId = healthcarePatientIds[i];

  sql.push(`INSERT OR IGNORE INTO cardiology_pacemakers (id, patient_id, company_id, manufacturer, model, serial_number, implant_date, implanting_doctor, implanting_hospital, device_type, leads_count, battery_status, battery_voltage, estimated_longevity, last_interrogation_date, next_interrogation_date, pacing_mode, lower_rate, upper_rate, status, notes, created_at, updated_at) VALUES ('${id}', '${patientId}', '${COMPANY_ID}', '${randomChoice(['Medtronic', 'Boston Scientific', 'Abbott', 'Biotronik', 'MicroPort'])}', '${randomChoice(['Advisa MRI', 'ACCOLADE', 'Assurity MRI', 'Edora 8', 'Rega'])}', 'PM${randomInt(100000, 999999)}', '${randomDate(new Date('2018-01-01'), new Date('2024-06-01'))}', 'Dr. ${randomChoice(lastNames)}', '${randomChoice(['CHU Ibn Rochd', 'CHU Mohammed VI', 'Clinique Internationale', 'Hopital Cheikh Khalifa'])}', '${randomChoice(['single_chamber', 'dual_chamber', 'biventricular', 'icd'])}', ${randomChoice([1, 2, 3])}, '${randomChoice(['good', 'good', 'good', 'monitoring', 'low'])}', ${randomFloat(2.5, 3.1, 2)}, '${randomInt(3, 10)} ans', '${randomDate(new Date('2024-06-01'), new Date('2024-12-15'))}', '${randomDate(new Date('2025-01-01'), new Date('2025-12-31'))}', '${randomChoice(['VVI', 'DDD', 'DDDR', 'AAIR', 'CRT-D'])}', ${randomChoice([50, 55, 60])}, ${randomChoice([120, 130, 140, 150])}, 'active', 'Dispositif fonctionnel', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);`);
}

// CARDIOLOGY PACEMAKER INTERROGATIONS
console.log('Generating pacemaker interrogations...');
for (let i = 0; i < 150; i++) {
  const id = `cardio-interrog-${String(i + 1).padStart(5, '0')}`;
  const pacemakerId = randomChoice(pacemakerIds);

  sql.push(`INSERT OR IGNORE INTO cardiology_pacemaker_interrogations (id, pacemaker_id, company_id, interrogation_date, technician, doctor, battery_voltage, battery_impedance, lead_impedance_atrial, lead_impedance_ventricular, sensing_atrial, sensing_ventricular, threshold_atrial, threshold_ventricular, percent_paced_atrial, percent_paced_ventricular, episodes_af, episodes_vt, episodes_vf, findings, adjustments_made, next_follow_up, created_at, updated_at) VALUES ('${id}', '${pacemakerId}', '${COMPANY_ID}', '${randomDate(new Date('2024-01-01'), new Date('2024-12-15'))}', 'Tech. ${randomChoice(lastNames)}', 'Dr. ${randomChoice(lastNames)}', ${randomFloat(2.5, 3.1, 2)}, ${randomFloat(400, 600, 0)}, ${randomFloat(400, 700, 0)}, ${randomFloat(400, 700, 0)}, ${randomFloat(2.0, 5.0, 1)}, ${randomFloat(5.0, 15.0, 1)}, ${randomFloat(0.5, 1.5, 2)}, ${randomFloat(0.5, 1.5, 2)}, ${randomFloat(0, 100, 1)}, ${randomFloat(0, 100, 1)}, ${randomInt(0, 10)}, ${randomInt(0, 3)}, ${randomInt(0, 1)}, '${escapeSQL(randomChoice(['Parametres stables', 'Batterie en bon etat', 'Seuils normaux', 'Quelques ESV detectees', 'Episodes FA courts']))}', '${randomChoice(['Aucun', 'Ajustement sensibilite', 'Modification frequence base', 'Aucun changement'])}', '${randomDate(new Date('2025-01-01'), new Date('2025-12-31'))}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);`);
}

// CARDIOLOGY STENTS
console.log('Generating cardiology stents...');
for (let i = 0; i < 60; i++) {
  const id = `cardio-stent-${String(i + 1).padStart(4, '0')}`;
  const patientId = healthcarePatientIds[i % 50];

  sql.push(`INSERT OR IGNORE INTO cardiology_stents (id, patient_id, company_id, implant_date, implanting_doctor, implanting_hospital, stent_type, manufacturer, model, diameter, length, location, lesion_type, pre_stenosis, post_stenosis, timi_flow_pre, timi_flow_post, dual_antiplatelet_end_date, notes, created_at, updated_at) VALUES ('${id}', '${patientId}', '${COMPANY_ID}', '${randomDate(new Date('2018-01-01'), new Date('2024-06-01'))}', 'Dr. ${randomChoice(lastNames)}', '${randomChoice(['CHU Ibn Rochd', 'CHU Mohammed VI', 'Clinique Internationale', 'Hopital Cheikh Khalifa'])}', '${randomChoice(['DES', 'BMS', 'BVS'])}', '${randomChoice(['Medtronic', 'Abbott', 'Boston Scientific', 'Biotronik', 'Terumo'])}', '${randomChoice(['Xience', 'Resolute', 'Synergy', 'Orsiro', 'Ultimaster'])}', ${randomFloat(2.5, 4.0, 1)}, ${randomInt(15, 38)}, '${randomChoice(['LAD', 'LCX', 'RCA', 'LM', 'D1', 'OM1', 'PDA'])}', '${randomChoice(['A', 'B1', 'B2', 'C'])}', ${randomInt(70, 99)}, ${randomInt(0, 10)}, '${randomChoice(['0', '1', '2'])}', '3', '${randomDate(new Date('2025-01-01'), new Date('2026-06-01'))}', 'Procedure sans complication', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);`);
}
...
for (let i = 0; i < 400; i++) {
  ...
  sql.push(`INSERT OR IGNORE INTO healthcare_consultations (id, patient_id, company_id, module, consultation_date, consultation_time, consultation_type, doctor_name, chief_complaint, diagnosis, treatment_plan, notes, follow_up_date, status, created_at, updated_at) VALUES ('${id}', '${patientId}', '${COMPANY_ID}', '${module}', '${randomDate(new Date('2024-01-01'), new Date('2024-12-15'))}', '${String(randomInt(8, 17)).padStart(2, '0')}:${String(randomInt(0, 59)).padStart(2, '0')}', '${randomChoice(['routine', 'urgent', 'follow_up'])}', 'Dr. ${randomChoice(lastNames)}', '${escapeSQL(randomChoice(['Douleur', 'Controle', 'Suivi', 'Bilan']))}', '${escapeSQL(randomChoice(['Stable', 'Amelioration', 'A surveiller']))}', 'Surveillance', 'Consultation standard', '${randomDate(new Date('2025-01-01'), new Date('2025-06-30'))}', 'completed', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);`);
}
...
const medications = [
  ...
  { name: 'Latanoprost', module: 'ophthalmology', dosage: '0.005%', frequency: '1x/jour', route: 'collyre' },
  { name: 'Timolol', module: 'ophthalmology', dosage: '0.5%', frequency: '2x/jour', route: 'collyre' }
];

for (let i = 0; i < 200; i++) {
  const id = `hc-med-${String(i + 1).padStart(5, '0')}`;
  const med = randomChoice(medications);
  const patientId = med.module === 'cardiology'
    ? healthcarePatientIds[i % 100]
    : healthcarePatientIds[50 + (i % 100)];

  sql.push(`INSERT OR IGNORE INTO healthcare_medications (id, patient_id, company_id, module, medication_name, dosage, frequency, route, start_date, prescribing_doctor, indication, status, notes, created_at, updated_at) VALUES ('${id}', '${patientId}', '${COMPANY_ID}', '${med.module}', '${med.name}', '${med.dosage}', '${med.frequency}', '${med.route}', '${randomDate(new Date('2023-01-01'), new Date('2024-06-01'))}', 'Dr. ${randomChoice(lastNames)}', 'Traitement de fond', 'active', 'Prescription active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);`);
}

// HEALTHCARE ALERTS
console.log('Generating healthcare alerts...');
for (let i = 0; i < 80; i++) {
  const id = `hc-alert-${String(i + 1).padStart(5, '0')}`;
  const patientId = healthcarePatientIds[i % 150];
  const module = randomChoice(['cardiology', 'ophthalmology', 'dialysis']);

  sql.push(`INSERT OR IGNORE INTO healthcare_alerts (id, patient_id, company_id, module, alert_type, severity, title, description, triggered_at, status, created_at, updated_at) VALUES ('${id}', '${patientId}', '${COMPANY_ID}', '${module}', '${randomChoice(['lab_critical', 'medication_due', 'follow_up_required'])}', '${randomChoice(['low', 'medium', 'high'])}', '${randomChoice(['Resultat critique', 'Suivi requis', 'Anomalie'])}', 'Attention requise', CURRENT_TIMESTAMP, '${randomChoice(['active', 'acknowledged', 'resolved'])}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);`);
}

// Write SQL to file
const sqlContent = sql.join('\n');
fs.writeFileSync('/tmp/healthcare_seed_staging_v3.sql', sqlContent);
console.log(`\nGenerated ${sql.length} SQL statements`);
console.log('SQL file written to /tmp/healthcare_seed_staging_v3.sql');
